...
template <typename Category = file_locked_category, typename Functions = basic_file_functions<Category>>
	class basic_file {
		public:
			/// Represents an int which can hold a char or an EOF value:

			using char_int_t = int;
			...
			static constexpr char_int_t eof = EOF;
			...
			bool open(const std::filesystem::path &fileName, const enum_flags<open_mode> mode)
			{
			...
			bool open(const std::filesystem::path &fileName, open_modes mode, std::error_code *errorCode) noexcept
			{
